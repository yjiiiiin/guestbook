...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="yujin1234/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/yjiiiiin/guestbook.git'
            }
        }
        stage('Build') {
            steps {
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'yujin_b@gstkorea.net')
